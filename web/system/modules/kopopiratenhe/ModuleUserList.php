...
class ModuleUserList extends Module
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_userlist';
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### USER LIST ###';
	...
	protected function compile()
	{
        ...
        $objUser = $this->Database->prepare("SELECT * FROM tl_member WHERE disable=? ORDER BY lastname, firstname ASC;")->execute(0);
        while($objUser->next()) {        
            $arrUser = $objUser->row();
            ...
            $arrUser['url'] = sprintf($strUrl, $objUser->id);
            $arrUsers[] = (object) $arrUser;
        }
