 ...
 * @package    Controller
 */
class ModuleFractionList extends Module
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_fractionlist';
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### FRACTION LIST ###';
	...
	protected function compile()
	{
        ...
        $objFraction = $this->Database->prepare("SELECT * FROM tl_kopopi_fraktionen ORDER BY title ASC;")->execute();
        while($objFraction->next()) {
            $arrMembers  = null;        
...
//                     $arrParlamente[] = (object) $objParlament->row();
//                 }
//                 
...
//                 'imageUrl'    => sprintf($strUrl, $objUser->id),
//                 'caption'     => null,
...
//             $arrUser['avatar'] = $objImage;
            $arrFraction['url'] = sprintf($strUrl, $objFraction->id);
            $arrFractions[] = (object) $arrFraction;
        }
        
        $this->Template->Fractions = $arrFractions; 
	}
}
